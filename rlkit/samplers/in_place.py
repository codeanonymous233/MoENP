# this is kept the same as PEARL.
...
class InPlacePathSampler(object):
    ...
    ```
    """
    def __init__(self, env, policy, max_path_length, whether_moe):
        self.env = env
        self.policy = policy

        self.max_path_length = max_path_length
        self.whether_moe = whether_moe
    ...
    def obtain_samples(self, deterministic=False, max_samples=np.inf, max_trajs=np.inf, accum_context=True, resample=1):
        ...
        while n_steps_total < max_samples and n_trajs < max_trajs:
            ...
            if self.whether_moe: # mixture of experts pearl agent using multiple l.v. in the form of a tensor
                path['context'] = policy.z_experts.detach().cpu().numpy()
            else: # vanilla pearl agent using a single l.v.
